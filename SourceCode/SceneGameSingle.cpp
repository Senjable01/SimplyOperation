...
void SceneGameSingle::Update(float deltaTime)
{
    ...
    ImGui::Text(u8"ÉçÅ[Éhêî %d", loadNum);
    if (ImGui::Button(u8"ÉäÉçÅ[Éh"))
    {
        Scene::ChangeScene<SceneGameSingle>();
        ImGui::End();
        return;
    ...
    {
        ImGui::Checkbox(u8"SpriteBatchÇ≈ï`âÊ", &drawBrickByBatch);
    ...
    {
        ImGui::SliderInt(u8"ÉÇÉfÉãÇÃéÌóﬁ", &meshIndex, HAND::ROCK, HAND::PAPER);
